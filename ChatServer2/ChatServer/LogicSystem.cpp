...
void LogicSystem::DealMsg() {
	for (;;) {
		...
		if (_b_stop) {
			while (_msg_que.size()) {
				...
				if (call_back_iter == _fun_callbacks.end()) {
					_msg_que.pop();
					SPDLOG_WARN("message handler not found, msg_id={}", msg->_recv_msg_node->_msg_id); // 日志todo...
					continue;
				}
		...
		// 未关闭，则取出队首进行处理
		auto msg = _msg_que.front();
		SPDLOG_DEBUG("logic recv msg, msg_id={}", msg->_recv_msg_node->_msg_id); // 日志todo...
		auto call_back_iter = _fun_callbacks.find(msg->_recv_msg_node->_msg_id);
		if (call_back_iter == _fun_callbacks.end()) {
			_msg_que.pop();
			SPDLOG_WARN("message handler not found, msg_id={}", msg->_recv_msg_node->_msg_id); // 日志todo...
...
void LogicSystem::GetUserByUid(std::string uid, Json::Value& value) {
	...
	if (b_info) {
		...
		SPDLOG_DEBUG("search user cache hit, uid={}", uid);
