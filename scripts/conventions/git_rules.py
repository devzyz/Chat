...
VERBS = set("add allow align apply audit authenticate avoid bound build cache cancel check clarify clean close compare complete configure consolidate correct cover create decode define delete disable disambiguate document drain enable enforce ensure exclude expose extract fail fix generate guard handle implement improve include initialize isolate keep limit load make migrate normalize package parse pin preserve prevent record reduce refactor register reject release remove rename render replace require resolve restore retry return reuse revert route run save select separate serialize set share simplify skip sort split standardize stop support sync synchronize test track unify update use validate verify wire write".split())
