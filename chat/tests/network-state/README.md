...
`chat/CMakeLists.txt` registers `network_state_tests`. The existing `client-release` job exports JUnit XML to `build/test-results/client_unit.xml`, uploads it even after failure, and treats a missing report as an error.
...
- `QTcpSocket` loopback, connection/error signals, JSON handler outcomes, `HttpMgr`, and `UserMgr` state/signal transitions require additional injectable QObject boundaries. They are not silently skipped or represented by decoder assertions.
