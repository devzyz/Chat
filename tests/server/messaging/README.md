...
The eight in-memory tests use no socket, thread, or external service. Run `RunServerTests` or filter `MsgNodeTests.*:SendNodeTests.*:RecvNodeTests.*`; results are in `server_unit.xml`.
