...
Production: `ChatTransport.vcxproj`, shared by ChatServer and the quick test executables.
...
## Component contracts (11, server_component.xml)
...
Created/Closing send rejection, closed authentication rejection, Close-before-Start, and foreign-directory isolation.
...
TCP tests T09-SESSION-04, 05, and 13 respectively; they are not removed or tested through a second FSM.

## Loopback Integration contracts (13, server_integration.xml)

| ID | Contract |
| --- | --- |
| T09-SESSION-01 | Split header/body, adjacent frames, empty body, maximum receive body |
| T09-SESSION-02 | Oversized body rejected before dispatch |
| T09-SESSION-03 | Repeated Start creates only one read chain |
| T09-SESSION-04 | FIFO writes and admission semantics |
| T09-SESSION-05 | Exact MAX_SENDQUE capacity, close during write, buffer lifetime |
| T09-SESSION-06 | Replacement, stale kick, old close cannot erase new presence; UID cannot change |
| T09-SESSION-07 | Targeted kick during publication cancels binding and rolls back presence |
| T09-SESSION-08 | Storage unavailable cannot authenticate or register locally |
| T09-SESSION-09 | Slow presence cleanup does not block local close/send rejection |
| T09-SESSION-10 | Remote kick carries the previous Session ID |
| T09-SESSION-11 | Interrupted body read cannot dispatch or restart after close |
| T09-SESSION-12 | Stop drains accept/I/O, releases Session ownership and listening port |
| T09-SESSION-13 | Actual write failure closes and cleans matching presence once |
