...
本模块包含 7 个 `node:test` Unit 测试。Redis、SMTP、UUID 和 logger 均使用进程内 fake；不连接真实 Redis、
...
- 默认 logger 当前仍会输出收件邮箱和验证码；生产代码尚未实现日志脱敏，因此本模块只为测试注入静默 logger，
  不把“日志不含敏感信息”伪装成已通过契约。
- 当前 `bindAsync` callback 未检查 error 或实际端口便调用 `server.start()`；本次只增加无行为变化 seam，不把
  尚未实现的绑定 fail-fast 声明为通过。
