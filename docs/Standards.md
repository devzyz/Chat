...
- 接口注释 MUST 解释职责和作用；实现内注释解释原因、约束和生命周期，避免逐行翻译代码。
...
- 函数 MUST 准确表达业务动作或查询，操作使用动词和对象，布尔查询使用 Is/Has/Can/Should 等判断式名称（大小写随语言）；参数体现对象和单位。禁止拼写错误、模糊的 DoWork，以及混淆发送、提交、送达、已读。
- 框架重写、语言规定和外部协议接口保留规定名称；兼容性改名 MUST 单独迁移，内部接口遵循所属语言规范。
...
- 每个自有类、结构体、模板类 MUST 在声明正上方用中文说明职责；涉及所有权、线程或生命周期时补充约束。
- 每个函数 MUST 在头部说明作用，覆盖所有可见性、构造/析构、重载、信号/槽、测试函数及 default/delete 声明；每个重载说明差异，default/delete 说明意图。Lambda/匿名回调在表达式前简述作用。
- C++ 注释放在头文件声明处，作为唯一权威接口说明；无独立声明时放在定义处。实现无需重复，算法原因可就地补充。语言格式见 [Native](Native.md#接口注释) 与 [Languages](Languages.md)。
- 简单接口一行即可；非直观接口 MUST 补充参数、输出参数、返回值、失败语义，以及适用的副作用、线程、回调时机和有效期。空标签、过期参数名、仅重复名称的模板不算合格注释。
- 新增代码全部执行；修改旧函数时补齐该函数及所属类说明，修改行为同时更新注释。未触及的历史代码按 [治理计划](plans/CodeConventions.md) 分批补齐；第三方和生成代码不纳入整改。
