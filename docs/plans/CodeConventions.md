...
由 `AuditConventions` 生成到 `build/conventions/audit.json`。下表保留首次 N9 盘点的历史目录快照，
...
首次盘点时，这些文件发生变化会阻断增量检查，需要补相应语法支持和回归，不能新增静默排除。

首次 N9 交付核清目录并把全部发现回挂原工作包，当时上述历史欠账尚未逐个整改。
旧 UI 名称包含自动槽/字符串连接约束，N3 整改必须连同绑定、调用、元对象引用及 Qt 回归处理，
...
用户要求把未处理项实际完成后，按以下要求复核；上表保留首次盘点证据，当前结果由
[Status](../Status.md) 维护，不把历史发现数改写成当时已经通过。

- 完成上表回挂的生产、shared、测试和脚本命名及中文职责说明，不仅生成欠账清单。
- 增加 QTest、GTest、WINAPI、默认参数等已有合法语法支持及反例，未知或损坏语法仍失败。
- Qt 自动槽由被检查版本的 Designer 对象验证；框架、协议及外部 SDK 规定的名称保留，仍要求注释。
- 核查构造重载、资源关闭与归还、回执事务及异步回调说明，不能用统一模板描述不同实现。
- 全目录审计无诊断及解析失败；非支持语言逐项列明范围，不扩大“自动通过”的含义。
- 执行相关编译与回归，核对改名前后 C++ token、JS AST 和 PowerShell token；更新既有 PR 的实际验证记录。
